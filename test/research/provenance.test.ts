...
describe('validateUrls', () => {
  it('marks valid URLs as OK', async () => {
  ...
  it('marks invalid URLs as LIKELY_HALLUCINATED', async () => {
    const results = await validateUrls([
      'not-a-url',
      '://invalid',
    ]);
    expect(results).toHaveLength(2);
    expect(results.every((r) => r.status === 'LIKELY_HALLUCINATED')).toBe(true);
  });
  ...
  it('handles mixed valid and invalid URLs', async () => {
    ...
    expect(results[1]!.status).toBe('LIKELY_HALLUCINATED');
  });
});
